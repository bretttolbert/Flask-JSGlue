...
def app():
    ...
    app.add_url_rule("/", "case0", handler)
    app.add_url_rule("/<a>", "case1", handler)
    app.add_url_rule("/test/<a>", "case2", handler)
    app.add_url_rule("/<int:a>/test", "case3", handler)
    app.add_url_rule("/<int:a>/<int:b>", "case4", handler)
    app.add_url_rule("/<a>/data", "case5", handler)
    app.add_url_rule("/<b>/hello", "case5", handler)
    app.add_url_rule("/<a>/data/<b>", "case5", handler)
...
def test_url_for_6(app):
    ...
    assert (
        runUrlFor(
            ...
            "'case2', {a: 'hello', '_external': true, '_scheme': 'https'}",
        )
        == "https://localhost/test/hello"
    )
    assert (
        runUrlFor(
            client.get("/jsglue.js").data,
            "'case5', {a: 1, b: 9, '_external': true}",
        )
    ...
    assert (
        runUrlFor(
            client.get("/jsglue.js").data,
            "'case2', {a: 'hello', '_external': false, '_anchor': 'hello'}",
        )
        == "/test/hello#hello"
    )
    assert (
        runUrlFor(
            client.get("/jsglue.js").data,
            "'case5', {a: 1, b: 9, '_external': true, '_scheme': 'https', '_anchor': 'test'}",
        )
        == "https://localhost/1/data/9#test"
    )
...
def test_url_for_unknown_parameters(app):
    ...
    assert (
        runUrlFor(
            client.get("/jsglue.js").data,
            "'case0', {unknown1: 'yes', unknown2: 'no'}",
        )
    ...
    assert (
        runUrlFor(
            client.get("/jsglue.js").data,
            "'case0', {'_external': true, unknown: 'yes'}",
        )
        == "http://localhost/?unknown=yes"
    )
    assert (
        runUrlFor(
            client.get("/jsglue.js").data,
            "'case5', {a: 1, b: 9, '_external': true, '_scheme': 'https', '_anchor': 'test', unknown: 'yes'}",
        )
        == "https://localhost/1/data/9?unknown=yes#test"
    )
